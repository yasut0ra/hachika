...
current snapshot versionは33（commitment progressのmigrationはv31）。旧questionにactor情報がない場合、旧runtimeでproactive再質問に使われていたopenな`user_name / user_profile / relation`を`hachika -> user`として移行し、それ以外は`user -> hachika`として扱う。旧requestは`user -> hachika`として移行する。
