...
永続化単位は `HachikaSnapshot` です。現在の schema version は **34** です。
...
resident loopは`HACHIKA_METRICS_TIME_ZONE`の暦日ごとに一度、32%の確率で小さな外生イベントを判定します。発生と種類は日付 + `HACHIKA_INDIVIDUAL_ID`（未指定時はdata root）から決定されるため再現可能で、発生しなかった日も`lastDailyEventCheckDate`を保存して同日中の再抽選を防ぎます。発生時は`occurrence`としてrecent eventsへ加え、現在地のobject state・familiarity・last engagedにも小さな反応を残します。
...
- daily metrics: `<data-root>/metrics-log.jsonl`
- immutable daily archive: `<data-root>/archive-snapshots/YYYY-MM-DD.json`
- 保存形式: temporary file + rename による atomic write
- compatibility: 古い snapshot を load 時に version 34 へ hydrate / sanitize
...
resident loopはsnapshotとartifactの正常commit後に、日次の縦断計測をJSON Linesで追記します。constitutionの全set-point、plasticity、現在のattachment、urge baseline、aspiration、voice、journal件数、直近24時間のuser turn数に加え、snapshot version / revisionと実装revisionを保存します。同じ暦日は再起動や複数tickでも1行だけです。日付境界は`HACHIKA_METRICS_TIME_ZONE`（IANA timezone、既定`UTC`）、実装revisionはGitの`HEAD`または`HACHIKA_IMPLEMENTATION_REVISION`で指定できます。末尾の書き込みが中断しても、破損行を読み飛ばして次回の正常commitから記録を再開します。
...
`npm run experiment:check`はsecretを含まない実験manifestをcanonical JSONとしてSHA-256 fingerprint化し、凍結tagが現在のHEADを指すこと、worktreeがcleanであること、Node.jsとwall-clock runtime構成、暦日、LLM構成、個体ID・名前・data root・seedの一意性を検査します。manifest例は`docs/experiment-config.example.json`、birth記録様式は`docs/lab-notes/birth-record-template.md`です。API keyをmanifestへ含めると検査に失敗します。
...
長期運用ではcronまたはlaunchdから個体ごとにmaintenanceを実行します。正常時は0、stale / inactive / status欠損時は1で終了します。
